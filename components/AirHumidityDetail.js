...
const AirHumidityDetail = ({navigation}) => {
  
  const [devices, setDevice]= useState(0);
  const [chartData, setChartData] = useState([
           
    { barValues: [70] ,month: 'Mar' },
    { barValues: [70] ,month: 'Apr' },
  ...
  useEffect(()=>{
   ...
   async function getDeviceState() {
    try {
      ...
      const temperatureDevice = response.data.devices.find(device => device.deviceName === "airhumidity");
...
export default AirHumidityDetail;